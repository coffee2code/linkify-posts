...
class Linkify_Widget_Test extends WP_UnitTestCase {
	...
	public function test_widget_version() {
		$this->assertEquals( '006', c2c_LinkifyPostsWidget::version() );
	}
	...
	public function test_output_omits_unsafe_markup() {
		list( $widget, $config, $settings ) = $this->widget_init( array(
			'posts'   => array_slice( $this->post_ids, 0, 2 ),
		...
		$this->assertEquals(
			sprintf(
				'<div class="my-widget">alert("boom!");<h3>alert("boom!");Postsalert("boom!");</h3><ul><li>alert("boom!");%s</li><li>alert("boom!");<strong><span class="test">Hi</span></strong></li><li>%salert("boom!");</li></ul>alert("boom!");</div>',
				__c2c_linkify_posts_get_post_link( $this->post_ids[0] ),
				__c2c_linkify_posts_get_post_link( $this->post_ids[1] )
			),
			$this->get_results( $widget, $settings, $config )

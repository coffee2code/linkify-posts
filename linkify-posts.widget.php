 ...
 *
 * Copyright (c) 2011-2019 by Scott Reilly (aka coffee2code)
 *
...
class c2c_LinkifyPostsWidget extends WP_Widget {
	...
	public function __construct() {
		$this->title = __( 'Linkify Posts', 'linkify-posts' );
		...
		$widget_ops = array(
			...
			'description' => __( 'Converts a list of posts (by slug or ID) into links to those posts.', 'linkify-posts' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		foreach ( array_keys( $this->config ) as $key ) {
			$$key = apply_filters( 'linkify_posts_widget_'.$key, $instance[ $key ] );
		}
